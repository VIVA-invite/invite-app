...
export default function Activity() {
  ...
  const { startTime = invitationData.startTime, endTime = invitationData.endTime } =
    location.state || {};
  ...
  useEffect(() => {
    try {
      ...
      const clampTime = (v: number) => Math.min(Math.max(v, sM), eM);

      const restoreActs = (saved.activities || [])
        .map((a) => ({ ...a, time: clampTime(a.time) }))
  ...
  }, [resolvedStart, resolvedEnd, setActivities]);
    ...
    <main className="flex flex-col items-center justify-center min-h-screen bg-background px-4 gap-6 pad-6">
      <div className="flex w-full gap-6">
        ...
        >
          <div
            ref={containerRef}
            className="relative w-10 h-[520px] md:h-[600px] self-start -ml-4"
          >
            {/* Thin blue bar */}
            ...
            {activities.map((a) => (
              <DraggablePill
                ...
                y={timeToY(a.time)}
                onRemove={() =>
                  setActivities((prev) => prev.filter((p) => p.id !== a.id))
                }
              />
            ))}
          </div>
...
function DraggablePill({
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute z-20 text-xs font-medium text-gray-800"
        style={{
          left: "calc(100% + 8px)",
          top: "50%",
